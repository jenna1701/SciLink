...
def generate_tleap_script(
    ...
    # Source leaprc files based on composition
    ...
    # Load small molecule params
    if mol2_files:
        for info in mol2_files:
            name = info.get("name", "MOL")
            lines += [f"{name} = loadmol2 {info['mol2']}"]
    if frcmod_files:
    ...
    # Ions
    if neutralize:
        lines.append("addIonsRand SYS Na+ 0 Cl- 0")
        lines.append("")
