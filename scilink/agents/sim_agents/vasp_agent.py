...
class VaspInputAgent:
    ...
    def _load_skill(self, skill: str) -> dict:
        """
        Load a VASP domain skill.

        Parameters
        ----------
        skill : str
            Skill name (resolved from scilink/skills/vasp/) or path to a .md file.

        Returns
        -------
        dict with skill_name and skill_sections, or empty skill state on failure.
        """
        try:
            from ...skills.loader import load_skill
            parsed = load_skill(skill, domain="vasp")
            self.logger.info(f"Loaded VASP skill: {parsed.get('name', skill)}")
            return {"skill_name": parsed.get("name", skill), "skill_sections": parsed}
